...
protected function _enddoc()
{
   ...
   // Custom method to rotate text with optional color
function RotatedText($x, $y, $txt, $angle, $color = null)
{
    ...
    $this->Text($x, $y, $txt);
    $this->Rotate(0); // Reset the rotation back to normal

    // Reset color to black (or default) if color was set
    if ($color) {
        $this->SetTextColor(0, 0, 0); // Reset text color to black (or default)
    }
}


    // Custom method to handle rotation
	...
	function numberToWords($number) {
		...
		if ($number < 100) {
			$unit = $number % 10;
			$ten = $number - $unit;
			return $words[$ten] . ($unit ? ' ' . $words[$unit] : '');
		}
	
		if ($number < 1000) {
			$hundred = floor($number / 100);
			$remainder = $number % 100;
			return $words[$hundred] . ' Hundred' . ($remainder ? ' and ' . numberToWords($remainder) : '');
		}
		...
		if ($number < 1000000000) {
			...
			return numberToWords($million) . ' Million' . ($remainder ? ' ' . numberToWords($remainder) : '');
		}
	
		if ($number < 1000000000000) {
			$billion = floor($number / 1000000000);
			$remainder = $number % 1000000000;
			return numberToWords($billion) . ' Billion' . ($remainder ? ' ' . numberToWords($remainder) : '');
		}
