...
<script>
    ...
    $.ajax({
        type: "GET",
        url: "modules/invoice/get_sales_transactions.php",
        success: function(response) {
            var salesTransactions = JSON.parse(response);
            $("#salesTable tbody").empty();

            // Filter sales transactions with invoiceStatus = 'PAST DUE'
            var pastDueTransactions = salesTransactions.filter(function(transaction) {
                return transaction.invoiceStatus.toUpperCase() === 'PAST DUE';
            });

            pastDueTransactions.forEach(function(transaction) {
                var statusColorClass = getStatusColorClass(transaction.invoiceStatus);
