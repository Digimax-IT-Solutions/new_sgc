...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'upload') {
    ...
    if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
        ...
        try {
            ...
            $checkStmt = $connection->prepare("SELECT COUNT(*) FROM fs_classification WHERE name = ?");
            $insertStmt = $connection->prepare("INSERT INTO fs_classification (name) VALUES (?)");
...
if (get('action') === 'delete') {
    $id = get('id');
    $fs = FsClassification::find($id);

    if ($fs) {
        $fs->delete();
        flashMessage('delete_uom', 'FS Classification deleted successfully', FLASH_SUCCESS);
    } else {
        flashMessage('delete_uom', 'Invalid FS', FLASH_ERROR);
    }
    redirect('../../fs_classification');
}
...
if (post('action') === 'add') {
    ...
    try {
        FsClassification::add($name);
        flashMessage('add_fs_classification', 'FS Classification added successfully.', FLASH_SUCCESS);
    } catch (Exception $ex) {
        flashMessage('add_fs_classification', $ex->getMessage(), FLASH_ERROR);
    }
