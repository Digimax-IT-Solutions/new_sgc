...
$page = 'open_invoice'; // Set the variable corresponding to the current page
?>
...
<div class="main">
        ...
        <div class="container-fluid p-0">
            <div class="row mb-4">
                <div class="col-12 d-flex justify-content-between align-items-center">
                    <div>
                        <h1 class="h3"><strong>Create Open Invoice</strong></h1>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="dashboard">Dashboard</a></li>
                                <li class="breadcrumb-item"><a href="open_invoice">Open Invoices</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Create Open Invoice</li>
                            </ol>
                        </nav>
                    </div>
                    <div>
                        <a href="open_invoice" class="btn btn-secondary">
                            <i class="fas fa-arrow-left"></i> Back to Open Invoices
                        </a>
                    </div>
                </div>
            </div>

            <form id="openInvoiceForm" action="api/open_invoice_controller.php?action=add" method="POST">
                    ...
                    <div class="col-12 col-lg-8">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">Open Invoice Details</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-2">
                                    ...
                                    <!--Open Invoice Details Section -->
                                    <div class="col-12 mt-3 mb-3">
                                        <h6 class="border-bottom pb-2">Invoice Information</h6>
                                    </div>
                                    <div class="col-md-3 open_invoice-details">
                                        <label for="open_invoice_number" class="form-label">Open Invoice Number</label>
                                        <input type="text" class="form-control form-control-sm" id="open_invoice_number"
                                            name="open_invoice_number" required>
                                    </div>
                                    <div class="col-md-3 open_invoice-details">
                                        <label for="open_invoice_date" class="form-label">Open Invoice Date</label>
                                        <input type="date" class="form-control form-control-sm" id="open_invoice_date"
                                            name="open_invoice_date" value="<?php echo date('Y-m-d'); ?>" required>
                                    </div>
                                    <div class="col-md-3 open_invoice-details">
                                        <label for="terms" class="form-label">Terms</label>
                                        <select class="form-select form-select-sm" id="open_invoice_terms" name="terms">
                                    ...
                                    <div class="col-md-3 open_invoice-details">
                                        <label for="open_invoice_due_date" class="form-label">Due Date</label>
                                        <input type="date" class="form-control form-control-sm" id="open_invoice_due_date"
                                            name="open_invoice_due_date" required>
                                    </div>
                                    <div class="col-md-3 open_invoice-details">
                                        <label for="open_invoice_account_id" class="form-label">Account</label>
                                        <select class="form-select form-select-sm" id="open_invoice_account_id"
                                            name="open_invoice_account_id" required>
    ...
    <div id="loadingOverlay" style="display: none;">
        ...
        <div class="message">Processing Open Invoice</div>
    </div>
</div>
...
<script>
    document.getElementById('open_invoice_terms').addEventListener('change', function() {
        ...
        var open_invoiceDate = document.getElementById('open_invoice_date').value;
        console.log("Invoice Date:", open_invoiceDate);

        if (open_invoiceDate && daysDue !== undefined) {
            var dueDate = new Date(open_invoiceDate);
            ...
            document.getElementById('open_invoice_due_date').value = `${year}-${month}-${day}`;
...
<script>
    ...
    document.getElementById('openInvoiceForm').addEventListener('submit', function() {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var open_invoiceAccountSelect = document.getElementById('open_invoice_account_id');
        ...
        function updateLabelAndOptions() {
            ...
            open_invoiceAccountSelect.innerHTML = '';
            ...
            accounts.forEach(function(account) {
                ...
                if (isCashSales) {
                    ...
                    if (account.account_type === "Bank" || account.account_description.toLowerCase().includes("undeposited")) {
                        open_invoiceAccountSelect.appendChild(option);
                    }
                } else {
...
<script>
    $(document).ready(function() {
        ...
        $('button[type="reset"]').on('click', function(e) {
            ...
            $('#open_invoice_date').val(new Date().toISOString().split('T')[0]);
        ...
        function saveDraft() {
            ...
            const formData = new FormData($('#openInvoiceForm')[0]);
            ...
            $.ajax({
                url: 'api/open_invoice_controller.php',
                type: 'POST',
                dataType: 'json',
                data: formData,
                processData: false,
        ...
        $('#open_invoice_account_id').select2({
            theme: 'classic', // Use 'bootstrap-5' for Bootstrap 5, 'bootstrap-4' for Bootstrap 4
            width: '100%',
            placeholder: '',
            allowClear: false
        ...
        // INVOICE TERMS 
        // $('#open_invoice_terms').change(function () {
        //     var terms = $(this).val();
        //     var deliveryDate = calculateDeliveryDate(terms);
        //     $('#open_invoice_due_date').val(deliveryDate);
        // });
        ...
        $('#openInvoiceForm').submit(function(event) {
            ...
            // Log the open_invoice_account_id value
            console.log('Submitting open_invoice_account_id:', $('#open_invoice_account_id').val());
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed && response.open_invoiceId) {
                                printInvoice(response.open_invoiceId, 1); // Pass 1 for initial print
                            } else {
                                location.reload();
                            }
                        });
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error saving open_invoice: ' + (response.message || 'Unknown error')
                        });
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'An error occurred while saving the open_invoice: ' + textStatus
                    });
                }
            });
        });

        function printInvoice(open_invoiceId, printStatus) {
            $.ajax({
                url: 'api/open_invoice_controller.php',
                type: 'POST',
                dataType: 'json',
                data: {
                    ...
                    open_invoice_id: open_invoiceId,
                    print_status: printStatus
                },
                success: function(response) {
                    if (response.success) {
                        console.log('Print status updated, now printing open_invoice:', open_invoiceId);

                        // Construct the print content URL
                        const printContentUrl = `print_open_invoice?action=print&id=${open_invoiceId}`;

                        // Open a new window with the print content
                        const printWindow = window.open(printContentUrl, 'PrintWindow', 'width=1000,height=700');

                        // Disable a button or perform another action while printing (optional)
