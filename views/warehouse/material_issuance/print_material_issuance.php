...
try {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($material_issuance) {

            // Get the check ID from the request
            $mis_id = $_GET['id']; // Assuming the ID is passed as a query parameter

            // Fetch purchase data based on the provided ID
            $material_issuance = MaterialIssuance::find($mis_id);
            ...
            if ($material_issuance) {
                ...
                } elseif ($material_issuance->status == 4) {
                ...
                if ($material_issuance->print_status == 1) {
                ...
                $pdf->SetFont('Arial', '', 9);
                $pdf->Cell(0, 10, 'PR No. ' . $material_issuance->mis_no, 0, 1, 'R');
                $pdf->Ln(5);
                ...
                $textLeft = 'Date: ' . $material_issuance->date;
                $pdf->Cell($maxWidth / 2 - $padding, $lineHeight, $textLeft, 0, 1, 'L');


                $textRight = 'Requesting Section: ' . $material_issuance->location;
                $pdf->Cell($maxWidth / 2 - $padding, $lineHeight, $textRight, 0, 0, 'L');

                $textRight = 'Date Required: ' . $material_issuance->date;
                ...
                $pdf->SetFont('Arial', '', 9);
                if ($material_issuance) {
                    foreach ($material_issuance->details as $detail) {
                        $startY = $pdf->GetY();
                        $currentX = $pdf->GetX();
                        ...
                        $pdf->SetXY($currentX + 110, $startY);
                        $pdf->Cell(20, 5, $detail['quantity'], 0, 0, 'C');
                        $pdf->Cell(20, 5, $detail['name'], 0, 0, 'C');// Assuming 'Ordered Quantity' is the same as 'Quantity'
                ...
                $pdf->MultiCell($cellWidth, $lineHeight, 'Memo: ' . $material_issuance->purpose, 0, 'L', false);
